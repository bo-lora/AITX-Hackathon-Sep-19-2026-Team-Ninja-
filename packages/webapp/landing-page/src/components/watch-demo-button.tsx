  ...
  async function loadSample() {
    ...
    try {
      ...
      if (!response.ok || !payload.workflowUrl) {
        throw new Error(
          payload.error || "Could not open the taught skill. Is the skill manager running on :43124?",
        );
      }
      window.location.href = payload.workflowUrl;
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not open the taught skill.");
      setPending(false);
    }
  }
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      <button
        ...
        onClick={loadSample}
        disabled={pending}
        className="inline-flex items-center gap-3 text-left font-semibold text-navy focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-navy disabled:opacity-40"
      >
        ...
        {pending ? "Opening taught skill…" : "Skip train — open a taught skill"}
      </button>
